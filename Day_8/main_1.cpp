...
int main()
{
    ...
    {
        if (graph.find(point_to_point_distances[i].A) == graph.end())
            graph[point_to_point_distances[i].A] = vector<point> ();
        graph[point_to_point_distances[i].A].push_back(point_to_point_distances[i].B);

        if (graph.find(point_to_point_distances[i].B) == graph.end())
            graph[point_to_point_distances[i].B] = vector<point> ();
        graph[point_to_point_distances[i].B].push_back(point_to_point_distances[i].A);
    }

    // Finding all subgroups with repeated DFS:
    vector<int> circuits; 
    ...
    cout << circuits[0] * circuits[1] * circuits[2] << endl;
